...
function TicketInfo() {
  ...
  useEffect(async() => {
    await fetch(`http://localhost:8080/updates`)
      .then(response => response.json())
  ...
  useEffect(async () => {
   await fetch(`http://localhost:8080/help_desk_users`)
      .then(response => response.json())
  ...
  // useEffect(() => {
  //   console.log('2Tickets: ', ticket)
  //   console.log('2Updates: ', updates)
  // })
  ...
  const handleSubmit = (event) => {
    ...
    // PATCH request to insert into ticket_updates table
  ...
  return (
    <div>
      ...
      {/* <p>Currently assigned to: {ticket[0].assigned_to}</p> */}
